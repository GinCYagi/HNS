...
class RoleRunner
  RUNTIME_DIRS = %w[work reviews results state decisions].freeze
  BOOLEAN_KEYS = %w[canon_change_requested decision_required acceptance_test_change_requested].freeze
  ...
  def run
    ...
    save_state("running", 0, "protected_baseline" => protected_baseline_receipt)
    ...
    1.upto(max_rounds) do |round|
      ...
      protection_reason = protection_escalation_reason(check_protected_paths("after_implementation"))
      return finish("escalated", round, protection_reason, implementation, nil, nil) if protection_reason

      implementation_reason = implementation_escalation_reason(implementation)
      return finish("escalated", round, implementation_reason, implementation, nil, nil) if implementation_reason

      test_result = run_tests(round)
      protection_reason = protection_escalation_reason(check_protected_paths("after_tests"))
      return finish("escalated", round, protection_reason, implementation, nil, test_result) if protection_reason

      failures = test_result["passed"] ? 0 : failures + 1
      save_state("running", round, "test" => test_result, "implementation" => implementation,
                 "protected_path_check" => @last_protection_check)
      ...
      protection_reason = protection_escalation_reason(check_protected_paths("after_review"))
      return finish("escalated", round, protection_reason, implementation, review, test_result) if protection_reason
    ...
    begin
      return finish("failed", current_round, "runtime_error", nil, nil, nil,
                    "error" => { "class" => e.class.name, "message" => e.message })
    rescue StandardError => finish_error
      warn finish_error.full_message
      return 2
    end
  end
  ...
  def prepare
    (["tasks"] + RUNTIME_DIRS).each { |name| FileUtils.mkdir_p(File.join(@root, "ops", name)) }
  ...
  def validate!
    ...
    missing_task_keys = %w[header objective scope acceptance].reject { |key| @task.key?(key) }
    raise "task missing keys: #{missing_task_keys.join(', ')}" unless missing_task_keys.empty?

    roles = @runtime_map.fetch("roles")
    %w[HNS-Imp-local HNS-Rev-local].each do |role|
      command = roles.dig(role, "command")
      raise "missing command mapping for #{role}" unless string_array?(command)
    end

    auto = @policy.fetch("auto_continue_severities")
    escalated = @policy.fetch("escalate_severities")
    raise "invalid auto_continue_severities" unless string_array?(auto)
    raise "invalid escalate_severities" unless string_array?(escalated)
    protected = @policy.fetch("protected_paths")
    %w[canonical acceptance].each do |category|
      raise "invalid protected_paths.#{category}" unless string_array?(protected[category])
    end
  end

  def string_array?(value)
    value.is_a?(Array) && value.all? { |entry| entry.is_a?(String) && !entry.empty? }
  end
  ...
  def implementation_prompt(round)
    ...
    <<~PROMPT
      ...
      Do not push, merge, approve canonical changes, change acceptance tests, or issue decisions.
      Protected canonical and acceptance paths are checked by content hash after your run.
      Round: #{round}
      Task:
      #{@task.to_yaml}
  ...
  def invoke_role(role, prompt, round, kind)
    ...
    File.write("#{prefix}.request.yaml", { "role" => role, "round" => round, "kind" => kind, "prompt" => prompt }.to_yaml)
  ...
  def validate_runtime_output!(kind, value)
    ...
    boolean_keys = BOOLEAN_KEYS + (kind == "review" ? %w[scope_unchanged conflicting_recommendations] : [])
    invalid = boolean_keys.reject { |key| value[key] == true || value[key] == false }
    raise "runtime output non-boolean keys: #{invalid.join(', ')}" unless invalid.empty?
  ...
  def run_tests(round)
    ...
    raise "test command must be a string or string array" unless string_array?(command)
  ...
  def protected_snapshot
    @policy.fetch("protected_paths").each_with_object({}) do |(category, patterns), categories|
      files = patterns.flat_map { |pattern| Dir.glob(File.join(@root, pattern), File::FNM_DOTMATCH) }
      categories[category] = files.uniq.select { |path| File.file?(path) }.sort.to_h do |path|
        [relative(path), Digest::SHA256.file(path).hexdigest]
      end
    end
  end

  def protected_baseline_receipt
    @protected_baseline.transform_values do |files|
      { "file_count" => files.length, "sha256" => Digest::SHA256.hexdigest(files.sort.to_h.to_yaml) }
    end
  end

  def check_protected_paths(stage)
    current = protected_snapshot
    changes = @protected_baseline.flat_map do |category, before|
      after = current.fetch(category, {})
      (before.keys | after.keys).sort.each_with_object([]) do |path, detected|
        next if before[path] == after[path]
        change = if !before.key?(path)
                   "added"
        ...
        detected << { "category" => category, "path" => path, "change" => change,
                      "before_sha256" => before[path], "after_sha256" => after[path] }
      end
    end
    @last_protection_check = { "stage" => stage, "passed" => changes.empty?, "changes" => changes }
  end

  def protection_escalation_reason(check)
    return nil if check["passed"]
    categories = check.fetch("changes").map { |change| change.fetch("category") }
    categories.include?("canonical") ? "protected_canonical_change" : "protected_acceptance_change"
  end

  def escalation_reason(review)
    severity = review.fetch("review_severity")
    return "review_severity" if @policy.fetch("escalate_severities").include?(severity)
    return "unhandled_review_severity" unless @policy.fetch("auto_continue_severities").include?(severity)
    return "scope_changed" unless review["scope_unchanged"]
    (BOOLEAN_KEYS + %w[conflicting_recommendations]).find { |key| review[key] }
  end

  def implementation_escalation_reason(implementation)
    BOOLEAN_KEYS.find { |key| implementation[key] }
  end
  ...
  def write_gin_request(round, reason, implementation, review, test_result)
    ...
    request = {
      "document_type" => "DecisionRequest", "run_id" => @run_id, "status" => "awaiting_gin",
      "escalation_reason" => reason, "round" => round, "task_path" => relative(@task_path),
      "requested_from" => "Gin", "requested_action" => "Decision or approval", "decision_issued" => false,
      "implementation" => implementation, "review" => review, "test" => test_result,
      "protected_path_check" => @last_protection_check, "created_at" => Time.now.utc.iso8601
    }
